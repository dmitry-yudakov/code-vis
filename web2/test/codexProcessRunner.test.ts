...
import { CodexProcessRunner } from '@/lib/server/codexProcessRunner';
import { checkCodex } from '@/lib/server/codexPreflight';
import { buildCodexAppServerArgs, codexTurnSecurity } from '@/lib/server/codexInvocation';
...
describe.sequential('CodexProcessRunner', () => {
  ...
  async function run(options: RunOptions = {}) {
    const directory = await mkdtemp(path.join(os.tmpdir(), 'web2-codex-'));
  ...
  it('interrupts on cancellation and timeout before closing the child', async () => {
    ...
    const cancelledDirectory = await mkdtemp(path.join(os.tmpdir(), 'web2-codex-cancel-'));
