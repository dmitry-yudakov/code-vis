...
function classifyCodexFailure(
  ...
  if (error instanceof RpcResponseError && (error.code === -32601 || /requires experimentalapi|invalid params/.test(normalized))) {
    return new AgentRunError('unsupported-flags', 'The installed Codex version does not support Cartograph\'s required App Server protocol.', 'not-sent', false);
  }
  ...
  async run(input: AgentProcessRun): Promise<AgentProcessResult> {
    ...
    return new Promise<AgentProcessResult>((resolve, reject) => {
      ...
      const respondUnsupported = (id: RpcId) => write({
        id,
        error: { code: -32601, message: 'This server request is disabled by Cartograph.' },
      });
      const scheduleKill = () => {
      ...
      const emitItem = (item: JsonRecord, completed: boolean) => {
        ...
        if (type === 'commandExecution') {
          ...
          stopWith(new AgentRunError(
            'unsupported-flags',
            `Codex attempted to use a capability Cartograph disabled (${type}).`,
      ...
      void (async () => {
        try {
          await request('initialize', {
            clientInfo: { name: 'cartograph_web2', title: 'Cartograph web2', version: '0.1.0' },
            capabilities: null,
          });
          notify('initialized');
          ...
          if (isolationIssue) {
            throw new AgentRunError(
              'unsupported-flags',
              `${isolationIssue} Disable it in Codex before using this provider in Cartograph.`,
              'not-sent',
              false,
            );
          }
          ...
          if (scopedIsolationIssue) {
            throw new AgentRunError(
              'unsupported-flags',
              `${scopedIsolationIssue} Disable it in Codex before using this provider in Cartograph.`,
              'not-sent',
              false,
            );
          }
