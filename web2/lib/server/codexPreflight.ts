...
import type { ProviderHealth } from '@/lib/shared/types';
import {
      ...
      try {
        ...
        if (!issue && mcpServerNames) {
          ...
          const threadResult = record(await request('thread/start', {
            ...
            developerInstructions: 'Cartograph readiness probe. Do not use tools or external integrations.',
            ephemeral: true,
        ...
        if (issue) {
          finish({
            ...
            authenticated: true,
            supportedModes: [],
            message: `${issue} Disable it in Codex before using this provider in Cartograph.`,
          });
          return;
        }
        finish({
        ...
        finish({
          ...
          message: /unauthori[sz]ed|auth|login/i.test(message)
            ...
            : 'The installed Codex does not support Cartograph\'s required App Server protocol.',
        });
      }
    })();
  });
