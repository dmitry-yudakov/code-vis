...
import type { EvidenceResult } from '@/lib/shared/types';
...
export async function extractEvidence(source: string, projectRoot: string): Promise<EvidenceResult[]> {
  ...
  for (const match of source.matchAll(EVIDENCE_LINE)) {
    ...
    if (!relativePath || path.isAbsolute(relativePath) || relativePath.includes('\\') || relativePath.includes('\0')
      ...
      results.push({ elementId, location, status: 'outside-project', message: 'Evidence must use a safe repository-relative path.' });
      continue;
    }
    ...
    if (!contained(root, unresolved)) {
      results.push({ elementId, location, status: 'outside-project', message: 'Evidence resolves outside the project.' });
      continue;
    }
    try {
      ...
      if (!contained(root, filePath) || !details.isFile()) {
        results.push({ elementId, location, path: relativePath, status: 'outside-project', message: 'Evidence is not a regular project file.' });
        continue;
      }
