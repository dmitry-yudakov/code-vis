...
import { IMMERSIVE_CHAT_FONT_SIZE, IMMERSIVE_CHAT_LINE_HEIGHT, IMMERSIVE_CHAT_TEXT_WIDTH } from '@/features/diagram/spatial/immersiveTranscript';
...
export function paintConversationHistory(resource: ReturnType<typeof createConversationHistoryResource>, entries: HistoryEntry[], scroll: ConversationHistoryScrollState,
  ...
  try {
    ...
    try {
      ...
      context.fillStyle = palette[theme].sheet;
...
function drawConversationHistory(resource: ReturnType<typeof createConversationHistoryResource>, entries: HistoryEntry[], scroll: ConversationHistoryScrollState,
  ...
  context.fillStyle = scroll.newActivity ? colors.live : colors.muted;
  ...
  if (failure || entries.length === 0) {
    context.font = `${IMMERSIVE_CHAT_FONT_SIZE}px Arial, sans-serif`;
    context.fillStyle = failure ? colors.stopInk : colors.muted;
    ...
    context.textAlign = 'left';
    context.fillStyle = isUser ? colors.plotWash : colors.neutralWash;
    context.beginPath();
    context.roundRect(x, y + 52, width, lines.length * IMMERSIVE_CHAT_LINE_HEIGHT + 36, isUser ? [24, 24, 6, 24] : [24, 24, 24, 6]);
    context.fill();
