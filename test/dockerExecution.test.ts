...
describe('Docker execution contract', () => {
  ...
  it('migrates active and archived v3 records to Local without changing ids, revisions or native histories', async () => {
    ...
    try {
      expect(await reopened.getSession(active.id)).toEqual(active);
      expect(await reopened.getArchivedSession(archived.id)).toEqual(archived);
      const persisted = JSON.parse(await readFile(path.join(root, 'session-store-v2', 'archived-sessions', `${archived.id}.json`), 'utf8'));
      expect(persisted).toMatchObject({ version: 4, execution: 'local', revision: archived.revision });
